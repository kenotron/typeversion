...
export function getResolvedType(type: ts.Type, checker: ts.TypeChecker, program: ts.Program): string {
  ...
  if (type.getFlags() & ts.TypeFlags.Object) {
    ...
    if (objectType.objectFlags & ts.ObjectFlags.Reference) {
      ...
      const hasTypeArgs = referenceType.typeArguments && referenceType.typeArguments.length > 0;

      if (!hasTypeArgs) {
        return checker.typeToString(type);
      }

      const resolvedTypeArguments = referenceType.typeArguments?.map((type) => getResolvedType(type, checker, program));
  ...
  if (declSourceFile && program.isSourceFileFromExternalLibrary(declSourceFile)) {
    return checker.typeToString(type);
  }

  if (type.getFlags() & ts.TypeFlags.UnionOrIntersection) {
    const unionType = type as ts.UnionType | ts.IntersectionType;
    const resolvedTypes = unionType.types.map((type) => getResolvedType(type, checker, program));

    return resolvedTypes.join(unionType.getFlags() & ts.TypeFlags.Union ? " | " : " & ");
    ...
    const resolvedMembers = members
      ...
      .map((member) => {
        ...
        return `${member.name}: ${getResolvedType(memberType, checker, program)}`;
      });
